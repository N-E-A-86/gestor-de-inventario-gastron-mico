...
try {
    ...
    if (!apiKey) {
        throw new Error("La clave de API de Gemini no está configurada. Ejecuta 'firebase functions:config:set gemini.key=...'");
    }
    ai = new GoogleGenAI({ apiKey });
} catch (error) {
    logger.error("Error al inicializar la API de Gemini. Asegúrate de que la configuración exista y hayas desplegado las funciones.", error);
}

// Se exporta la función usando la sintaxis estándar de v1, compatible con el package.json actualizado.
export const procesarListaPrecios = functions.https.onCall(async (data: any, context: functions.https.CallableContext) => {
    ...
    if (!ai) {
        logger.error("La instancia de la IA de Gemini no está disponible. Revisa los logs de inicialización.");
        throw new functions.https.HttpsError('internal', 'El servicio de IA no está configurado correctamente.');
    }
    ...
    if (!pdfData || !inventario) {
        throw new functions.https.HttpsError('invalid-argument', 'La función debe ser llamada con "pdfData" y "inventario".');
    }
    ...
    const prompt = `
        ...
        Devuelve únicamente un array JSON con los artículos para los que encuentres un precio actualizado. No incluyas nada más en tu respuesta.
    ...
    try {
        ...
        throw new functions.https.HttpsError('internal', 'No se pudo analizar el documento con la IA.', error);
    }
});
